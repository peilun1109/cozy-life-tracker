...
class WebHomeV6 extends StatefulWidget {
  const WebHomeV6({
    super.key,
  ...
  @override
  State<WebHomeV6> createState() => _WebHomeV6State();
}

class _WebHomeV6State extends State<WebHomeV6> {
  @override
              ...
              Expanded(
                ...
                child: Column(
                  children: [
                    _SectionCard(
                      title: '生活出席小日曆',
                      subtitle: '保留格子感，但用更柔和的方式記錄每一天留下的痕跡。',
                      accentColor: AppTheme.petal,
                      child: _EntryHeatmap(entries: widget.appState.entries),
                    ),
                    const SizedBox(height: 18),
                    _SectionCard(
                      title: '未來一週安排',
                      subtitle: '像一排安靜的小便條，每一天都能放一點想做的事。',
                      accentColor: AppTheme.mist,
                      child: LayoutBuilder(
                        builder: (context, constraints) {
                          const gap = 12.0;
                          final cardWidth =
                              ((constraints.maxWidth - gap * 6) / 7).clamp(104.0, 180.0);

                          return Row(
                            crossAxisAlignment: CrossAxisAlignment.start,
                            children: [
                              for (final date in upcomingWeek)
                                Padding(
              ...
              Expanded(
                ...
                child: Column(
                  children: [
                    _SectionCard(
                      title: '本週焦點',
                      subtitle: '只留下最重要的幾件事，讓首頁維持安靜而清楚。',
                      accentColor: AppTheme.apricot,
                      child: Column(
                        children: [
                          _MiniStatCard(
                            title: '優先目標',
                            value: '${focusGoals.length}',
                            note: focusGoals.isEmpty ? '先放一個想靠近的小目標' : focusGoals.first.title,
                          ),
                          const SizedBox(height: 12),
                          _MiniStatCard(
                            title: '最近倒數',
                            value: '${countdownGoals.length}',
                            note: countdownGoals.isEmpty ? '目前沒有倒數中的目標' : countdownGoals.first.title,
                          ),
                        ],
                      ),
                    ),
                    ...
                    _SectionCard(
                      title: '最近的生活紀錄',
                      subtitle: '保留少量摘要就好，首頁不需要塞滿所有內容。',
                      accentColor: AppTheme.sage,
                      child: recentEntries.isEmpty
                          ? const _EmptyText('今天還沒有新的紀錄，先寫下一句話也很好。')
                          : Column(
  ...
  String _greetingText() {
    ...
    if (hour < 11) {
      return '早安，把今天想留下來的小片段慢慢收進拾光機。';
    }
    if (hour < 18) {
      return '下午好，給自己留一格空白，也留一件想做的小事。';
    }
    return '晚上好，記得把今天的心情和碎片好好放進來。';
  }
}
...
class _UpcomingDayCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final lines = [
      ...items,
      ...dueGoals.map((goal) => '目標提醒：${goal.title}'),
    ];
        ...
        children: [
          ...
          SizedBox(
            ...
            child: FilledButton.tonalIcon(
              onPressed: onAdd,
              icon: Icon(
                items.isEmpty ? Icons.add_rounded : Icons.edit_rounded,
                size: 16,
              ),
              label: FittedBox(
                fit: BoxFit.scaleDown,
                child: Text(items.isEmpty ? '新增安排' : '編輯安排'),
              ),
              style: FilledButton.styleFrom(
                ...
                backgroundColor: const Color(0xFFF7EAEF),
    ...
    return Column(
      ...
      children: [
        ...
        Row(
          ...
          children: [
            Text('最近 20 週的生活溫度', style: Theme.of(context).textTheme.bodyMedium),
            Row(
              children: [
...
class _HeatmapWeek extends StatelessWidget {
    ...
    return Padding(
      ...
      child: Column(
        children: List.generate(7, (index) {
          ...
          final label = date == null ? '' : '${DateFormat('M/d').format(date)}，$count 筆紀錄';
  ...
  Color _heatColor(int count) {
    if (count <= 0) return const Color(0xFFF7EEF0);
    if (count <= 2) return const Color(0xFFEFCFD8);
    return const Color(0xFFDFA2B4);
  }
}
